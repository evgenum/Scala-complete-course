...
trait TestServiceImpl extends UsefulService {
  ...
  def doSomeService() =
    execute(sql)
      .split(" ")
      .length //подсчитайте количество слов в результате execute
}
...
class Application(isTestEnv: Boolean) {
  ...
  val usefulService: UsefulService =
    if (isTestEnv)
      new SQLAPI("test db Resource")
      with TestServiceImpl //передайте "test db Resource" в качестве ресурсв в конструктор SQLAPI
    else
      new SQLAPI("production Resource")
      with ProductionServiceImpl //передайте "production Resource" в качестве ресурсв в конструктор SQLAPI
